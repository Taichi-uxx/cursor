#!/usr/bin/env python3
"""gen_bg.py — テキストを含まないバナー広告用「背景ビジュアル(BG)」生成スクリプト.

このスクリプトはバナー広告で使う *背景画像のみ* を生成する。
キャッチコピー・CTA・ロゴ・数値訴求等の文字は後段 (compose_banner.py +
Playwright) で HTML/CSS により正確な日本語で重ねるため、画像には一切
テキストを入れさせない。
...
正常時、標準出力には JSON 1 行のみを出す:
  {"images":[{"aspect":"square","path":"<abs>"},...],"provider":"<使用provider>"}
"""
...
# (square=正方形 / horizontal=横長 / vertical=縦長)
_OPENAI_SIZE = {
    "square": "1024x1024",
...
# プロンプト末尾に言語で付与する (gen_kv の sp/pc 言語ヒント方式を踏襲)。
_GEMINI_ASPECT_HINT = {
...
def _generate_gemini(prompt: str, size_key: str, count: int) -> list[bytes]:
    ...
    Gemini 画像系はサイズを明示パラメータで取らないため、アスペクト指示は
    プロンプト末尾に言語で付与する (square / horizontal / vertical)。
    """
    api_key = (
    ...
    aspect = _GEMINI_ASPECT_HINT[size_key]
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        prog="gen_bg.py",
        description="バナー広告用の背景ビジュアル(BG)を生成 (テキスト無し)。",
    )
    p.add_argument("--prompt", required=True, help="背景ビジュアルの説明 (文字指示は不要)")
    p.add_argument("--out-dir", required=True, help="出力ディレクトリ")
    p.add_argument("--n", type=int, default=1, help="各アスペクトあたりの生成枚数 (default 1)")
    p.add_argument(
        "--aspect",
        default="square",
        help=(
            "生成アスペクト (カンマ区切り)。許容: square,horizontal,vertical "
            "/ all で3種全部 (default square)"
        ),
    )
    p.add_argument(
        "--focal",
        ...
        default="center",
        help="見出しを重ねる余白の位置 (default center)",
    )
    return p.parse_args(argv)


_VALID_ASPECTS = ("square", "horizontal", "vertical")


def _resolve_aspects(raw: str) -> list[str]:
    """--aspect 引数を解釈 (順序保持・重複除去)。空/不正なら ValueError。"""
    tokens = [t.strip().lower() for t in (raw or "").split(",") if t.strip()]
    if not tokens:
        raise ValueError("--aspect が空です")
    if "all" in tokens:
        return list(_VALID_ASPECTS)
    unknown = [t for t in tokens if t not in _VALID_ASPECTS]
    if unknown:
        raise ValueError(
            f"未対応のアスペクト {unknown} が含まれています。"
            f"許容: {', '.join(_VALID_ASPECTS)}, all"
        )
    seen: set[str] = set()
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        aspects = _resolve_aspects(args.aspect)
    ...
    # プロバイダを順に試行 (1プロバイダで全アスペクト成功してから採用＝混在を防ぐ)
    ...
    for provider in chain:
        ...
        try:
            ...
            for aspect in aspects:
                ...
                images = generator(prompt, aspect, args.n)
                if not images:
                    raise ProviderError(f"{aspect}: 画像が生成されませんでした")
                for i, png in enumerate(images, start=1):
                    if not png:
                        raise ProviderError(f"{aspect}#{i}: 空の画像データを受信しました")
                    tmp.append((aspect, i, png))
            collected = tmp
            used = provider
            break
    ...
    # ここまで来たら採用プロバイダで全アスペクト成功。ファイル書き出し。
    results: list[dict[str, str]] = []
    try:
        for aspect, i, png in collected:
            dest = out_dir / f"bg_{aspect}_{i}.{_img_ext(png)}"
            dest.write_bytes(png)
            results.append({"aspect": aspect, "path": str(dest)})
    except OSError as e:
