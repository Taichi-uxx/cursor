...
- **サービス構成**: 二段階診断（各10問ずつ・全20問）
  - ①16タイプ診断（MBTIライク）：長所・強み・注意点・キャリアの方向性・適性がわかる
  - ②マッチング診断（適職診断／①の結果をもとに診断）：相性のいいSaaS商材・5年後までの年収シミュレーション・おすすめ求人
- **サービスURL**:
  - 16タイプ診断フォーム: https://form.mar-cari.jp/ng/answers/careermatrix/
  - マッチング診断フォーム: https://form.mar-cari.jp/ng/answers/matchingsurvey
- **サービス開始**: 16タイプ診断＝2026-01-23／マッチング診断＝2026-02-24
- **制作背景**: 母集団形成（潜在層〜準顕在層）／マーキャリのこれまでの実績をベースにAI活用で診断結果を提示＝転職支援の疑似体験（キャリアは多種多様なため精度・品質担保にAI活用。強く押すことへのこだわりなし＝表立って発信していない）
- **KGI**: 面談CV獲得
...
- **Google広告（リスティング）**: あり（月5万・完全一致＋インテントマッチ・検索クエリは「キャリア診断」「転職診断」中心）
- **Microsoft広告**: -
- **Meta広告**: あり（月20万・動画1本＋静止画風動画2本＋2026-09-10追加2本＝計5本、配信・友達登録の9割が動画経由）
- **LINE広告**: 停止済み（〜2026-08・友達登録単価100円台だが面談CVにつながらず）
- **デマンドジェン**: 停止済み（〜2026-03）
- **YouTube広告**: 停止

### 流入元（オーガニック含む）
- Meta広告／リスティング／LINE広告（8月まで）／デマンドジェン（3月まで）／YouTube広告／サービスサイト／学ぶ！Sa活／YouTube概要欄

### Meta配信設計（8月時点）
- **年齢**: 25-34歳
- **性別**: 男女
- **エリア**: 日本
- **興味関心**: マーケティング（ビジネス・ファイナンス）／SaaS／デジタルマーケティング／ソーシャルメディアマーケティング／オンライン広告／広告／営業（ビジネス・ファイナンス）／プロモーション／マーケティング戦略／キャリア（雇用）／仕事（キャリア）／求職活動（キャリア）／就職面接（キャリア）／求人サイト（キャリア）
- **業種**: 営業
- **ライフイベント**: 就業・転職
- ※興味関心・業種・ライフイベントはいずれかにマッチで掲載対象

### リスティング配信設計（8月時点）
- **年齢**: 25-34歳
- **性別**: 男女
- **エリア**: 日本
- **キーワード（すべて完全一致／インテントマッチ）**:
  - 完全一致: 「キャリアマトリックス診断」「Career Matrix 診断」
  - インテントマッチ: 「キャリア 診断」「転職 診断」「営業 キャリア 診断」
- **運用方針**: 通常リスティングの指名広告とカニバる可能性があるため、少額予算で診断特化KWのみ。基本的に「キャリアマトリックス診断」検索者を拾う建て付け
...
- 16タイプ診断フォーム: https://form.mar-cari.jp/ng/answers/careermatrix/
- マッチング診断フォーム: https://form.mar-cari.jp/ng/answers/matchingsurvey
- 参考資料: `260911_Career Matrix_振り返り.pdf`（寺本さん共有）
