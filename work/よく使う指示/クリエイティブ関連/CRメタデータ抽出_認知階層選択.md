...
- **pain_point**: ターゲットの悩み
- **level_of_awareness**: ターゲットの認知レベル。以下のリストから**英語のキー**を選択してください。
    - "Unaware_Latent" (無関心・潜在層)
    - "Problem_Aware" (問題認知層)
    - "Solution_Aware" (解決策認知層)
    - "Product_Aware" (商品認知層)
    - "Comparison_Stage" (比較検討層)
    - "Purchase_Ready" (購入直前層)
    - "Existing_Customer" (既存顧客層)
    - "Churn_Recovery" (休眠復帰層)
