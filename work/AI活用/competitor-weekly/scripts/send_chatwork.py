#!/usr/bin/env python3
"""Chatwork送信ヘルパー（competitor-weekly 用）。

環境変数（`/Users/apple/.cursor/設定まわり/taichi-tamura/.env`）:
  - CHATWORK_API_TOKEN            : Chatwork APIトークン
  - CHATWORK_ROOM_ID_COMPETITOR   : 競合ウォッチ専用ルームID（優先）
  - CHATWORK_ROOM_ID              : フォールバック
  - CHATWORK_TO_ACCOUNT           : (任意) 自分宛To指定
...
def main() -> int:
    ...
    room_id = (
        args.room_id
        or os.environ.get("CHATWORK_ROOM_ID_COMPETITOR")
    ...
    if not args.dry_run:
        if not token:
            ...
            return 1
        if not room_id:
            print("ERROR: CHATWORK_ROOM_ID_COMPETITOR / CHATWORK_ROOM_ID が未設定", file=sys.stderr)
            return 1
