...
Google広告のデータを取得し、キーワード単位の健康診断を実施します。
...
- ①キーワード（キーワードとマッチタイプの組み合わせ）の重複率が何％か（重複しているキーワードのコスト比率で算出）
- ②インテントマッチ（部分一致）のコスト比率が何％か
- ③品質スコア6以上のキーワードのコスト比率は何％か
- ④「広告の関連性」が平均より上のキーワードのコスト比率は何％か
- ⑤「LPの利便性」が平均より上のキーワードのコスト比率は何％か
- ⑥「推定クリック率」が平均より上のキーワードのコスト比率は何％か

### 前提
- 対象となるキーワードは対象期間で費用が出ているものに限定する
- 文言が一緒でもマッチタイプが異なるキーワードは別キーワードとして捉える
- ②のインテントマッチ判定はkeywordMatchTypeが「BROAD」であるかどうかで判別する
- ③〜⑥の品質スコア関連指標はテーブルにカラムが存在しない場合、ユーザーに通知の上スキップする
