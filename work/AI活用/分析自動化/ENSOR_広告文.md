...
Google広告のデータを取得し、広告文単位の健康診断を実施します。
...
- ①配信中の広告文は80%以上がユニークか（headlines の組み合わせが同一の広告を「重複」として判定し、ユニーク率を算出）
- ②配信中のLPが複数種類設定されているか（trackingUrlTemplateのユニーク数をカウント）

### 前提
- 対象となる広告文は対象期間で表示回数が1回以上あるものに限定する
- ①のユニーク判定はheadlines（広告見出し）の組み合わせが完全に同一かどうかで判断する。一部のみ異なる場合は別の広告文として扱う
- ②のLP種類はtrackingUrlTemplateで判別する。パラメータの違いのみの場合は同一LPとして扱う
