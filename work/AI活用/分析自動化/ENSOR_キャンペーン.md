...
Google広告のデータを取得し、キャンペーン単位の健康診断を実施します。
...
## 3. 分析対象の確認
分析対象となるキャンペーンをユーザーにヒアリングする（アカウント全体を分析する場合は「全部」と回答してもらう）。
...
### 前提
- 対象となるキャンペーンは対象期間で費用が出ているものに限定する
- 入札戦略が「手動CPC」以外（目標コンバージョン単価、目標広告費用対効果、コンバージョン数の最大化、クリック数の最大化等）の場合は「自動入札」として扱う
